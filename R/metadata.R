...
parse_column <- function(c, parent = NULL, generated = FALSE) {
  ...
  col <- list(
    ...
    .specified = specified_properties,
    parent = parent
  )
...
inherit_val <- function(col, name) {
  is_specified <- function(obj, property) {
    if (!is.null(obj$.specified)) return(property %in% obj$.specified)
    !is.null(obj[[property]]) && !identical(obj[[property]], "")
  }

  if (is_specified(col, name)) return(col[[name]])
  tbl <- col$parent
  if (!is.null(tbl)) {
    sch <- tbl$tableSchema
    if (!is.null(sch) && is_specified(sch, name)) return(sch[[name]])
    if (is_specified(tbl, name)) return(tbl[[name]])
    grp <- tbl$parent
    if (!is.null(grp) && is_specified(grp, name)) return(grp[[name]])
  }
...
#' @param md_url optional location of the metadata document, used as its base URL
#' @return logical
does_metadata_describe_file <- function(md, url, md_url = NULL) {
  if (is.null(md)) return(FALSE)
  
  abs_csv_url <- normalize_url_local(url)
  
  base_resource <- if (!is.null(md_url)) md_url else url
  meta_base_url <- if (is_url(base_resource)) {
    base_resource
  } else {
    dirname(normalizePath(base_resource, mustWork = FALSE))
  }
...
locate_metadata <- function(url, md_url = NULL) {
  ...
  if (is_url(url)) {
    ...
    if (!is.null(resp)) {
      ...
      if (!is.null(link_hdr)) {
        ...
        if (m[[1]][1] != -1) {
          ...
          if (!is.null(md) && does_metadata_describe_file(md, url, md_full_url)) {
            return(list(md = md, no_header = no_header, is_located = TRUE, md_url = md_full_url))
          } else if (!is.null(md)) {
    ...
    for (line in locs) {
      ...
      if (!is.null(md) && does_metadata_describe_file(md, url, md_url_try)) {
        return(list(md = md, no_header = no_header, is_located = TRUE, md_url = md_url_try))
      } else if (!is.null(md)) {
    ...
    if (file.exists(file_md_path)) {
      ...
      if (does_metadata_describe_file(md, url, file_md_path)) {
        return(list(md = md, no_header = no_header, is_located = TRUE, md_url = file_md_path))
      } else {
        warning(paste("File metadata does not describe the CSV file:", url))
      }
    }

    # csv-metadata.json is the standard directory-level fallback. Keep
    # metadata.json as a compatibility extension after the standard name.
    for (metadata_name in c("csv-metadata.json", "metadata.json")) {
      dir_md_path <- file.path(dirname(url), metadata_name)
      if (file.exists(dir_md_path)) {
        md <- get_json(dir_md_path)
        if (does_metadata_describe_file(md, url, dir_md_path)) {
          return(list(md = md, no_header = no_header, is_located = TRUE, md_url = dir_md_path))
        } else {
  ...
  return(list(md = md, no_header = no_header, is_located = FALSE, md_url = NULL))
}
...
read_table_csv <- function(table, strict = TRUE, validate = FALSE, lax = FALSE) {
  ...
  raw_df <- tryCatch({
    suppressMessages(
      readr::read_delim(
        ...
        comment = if (is.null(dialect$commentPrefix)) "" else dialect$commentPrefix,
        col_names = FALSE,
        col_types = readr::cols(.default = readr::col_character()),
        na = character(),
        skip = skip,
        skip_empty_rows = isTRUE(dialect$skipBlankRows),
        locale = loc,
  ...
  while (nrow(raw_df) > 0 && row_is_blank(raw_df[nrow(raw_df), , drop = TRUE])) {
  ...
  if (dialect$skipBlankRows) {
    is_blank <- apply(raw_df, 1, row_is_blank)
  ...
  column_matches_header <- function(col, hname) {
    title_items <- list()
    if (col$has_name) {
    ...
    if (col$has_titles) {
      if (is.character(col$titles)) {
        for (title in col$titles) {
          title_items[[length(title_items) + 1]] <- list(value = title, lang = "und")
        }
        ...
        if (is.null(names(col$titles))) {
          for (item in col$titles) {
            if (is.character(item)) {
              ...
              title_items[[length(title_items) + 1]] <- list(value = value, lang = lang)
            }
          }
        } else {
          for (lang in names(col$titles)) {
            values <- col$titles[[lang]]
            for (value in values) {
              title_items[[length(title_items) + 1]] <- list(value = value, lang = lang)
            }
          }
    ...
    any(vapply(title_items, function(item) {
      !is.null(item$value) && item$value == hname && languages_match(item$lang, table_lang)
    }, logical(1)))
  }
  ...
  for (j in seq_along(header_names)) {
    ...
    if (dialect$header && !is.null(schema) && length(regular_cols) > 0) {
      ...
      if (length(matching_indices) > 0) {
        col <- regular_cols[[matching_indices[1]]]
      }
    }
  ...
  if (nrow(raw_df) > 0) {
    ...
    col_required <- vapply(matched_cols, function(col) {
      isTRUE(inherit_val(col, "required"))
    }, logical(1))
    col_defaults <- lapply(matched_cols, function(col) {
      value <- inherit_val(col, "default")
      if (!is.null(value) && value != "") value else NULL
    })
    col_seps <- lapply(matched_cols, function(col) inherit_val(col, "separator"))
    col_datatypes <- lapply(matched_cols, function(col) {
      value <- inherit_val(col, "datatype")
      if (is.null(value)) parse_datatype(NULL) else value
    })
    ...
    for (j in seq_along(matched_cols)) {
      ...
      if (length(non_null_indices) > 0) {
        ...
        if (!is.null(col_sep) && col_sep != "") {
          ...
          # The overwhelmingly common case needs no per-cell exception
...
encode_key_values <- function(values) {
  # serialize() preserves component boundaries, types, classes, and NULL,
  # unlike delimiter-based concatenation. Base64 makes the raw key safe for
  # duplicated() and match().
  jsonlite::base64_enc(serialize(unname(values), NULL, version = 2))
}

format_key_values <- function(values) {
  paste(vapply(values, function(value) {
    if (is_null_key_value(value)) return("<null>")
...
check_primary_key <- function(table, rows) {
  ...
  pk_strs <- vapply(key_values, encode_key_values, character(1))
  
  # Check for nulls
  for (j in seq_along(pk_cols)) {
    null_idx <- which(vapply(
      key_values, function(values) is_null_key_value(values[[j]]), logical(1)
    ))
  ...
  if (any(dup_flags)) {
    ...
    unique_dup_indices <- dup_indices[!duplicated(pk_strs[dup_indices])]
    if (length(unique_dup_indices) > 100) {
      ...
      for (idx in unique_dup_indices) {
        warning(paste("duplicate primary key:", format_key_values(key_values[[idx]])))
      }
    }
  }
    ...
    for (fk in schema$foreignKeys) {
      ...
      for (cname in source_cols) {
        ...
        if (length(col_idx) > 0) {
          ...
          separator <- inherit_val(col, "separator")
        ...
        for (i in seq_along(tbl_rows)) {
          ...
          for (cname in source_cols) {
            ...
            if (is.null(val) || (is.list(val) && length(val) == 0)) {
              ...
              col_values[[cname]] <- unlist(val, recursive = FALSE, use.names = FALSE)
            } else if (is_list_val && is.vector(val) && length(val) > 1) {
              col_values[[cname]] <- val
            } else {
              col_values[[cname]] <- val
            }
          }
          
          grid <- expand.grid(col_values, stringsAsFactors = FALSE, KEEP.OUT.ATTRS = FALSE)
          grid_keys <- character()
          grid_values <- vector("list", nrow(grid))
          for (idx in seq_len(nrow(grid))) {
            row_vals <- lapply(grid[idx, , drop = FALSE], function(value) {
              if (length(value) == 1 && is.na(value)) NULL else value
            })
            grid_values[[idx]] <- row_vals
            grid_keys[idx] <- encode_key_values(row_vals)
          }
          
          all_null <- vapply(grid_values, function(values) {
            all(vapply(values, is_null_key_value, logical(1)))
          }, logical(1))
          if (all(all_null)) {
            if (strict) {
          ...
          for (idx in seq_along(grid_keys)) {
            if (all_null[idx]) {
              next
            }
            if (!(grid_keys[idx] %in% target_keys)) {
              warning(paste(
                format_key_values(grid_values[[idx]]),
                "not found in table", ref_resource
              ))
              success <- FALSE
            }
...
csvw <- function(url, md_url = NULL, validate = FALSE, lax = FALSE) {
  ...
  run_with_warnings <- function() {
    ...
    md <- locate_res$md
    metadata_url <- locate_res$md_url
    ...
    ctx_props <- extract_context_properties(md)
    base_resource <- if (!is.null(metadata_url)) metadata_url else url
    meta_base_url <- if (is_url(base_resource)) {
      base_resource
    } else {
      dirname(normalizePath(base_resource, mustWork = FALSE))
